...
def migrate(c):
    ...
    """
    with Builder(c):
        docker_compose_run(c, 'php bin/console doctrine:database:create --if-not-exists', no_deps=True)
        docker_compose_run(c, 'php bin/console doctrine:migration:migrate -n --allow-no-migration', no_deps=True)
...
def migration(c):
    ...
    """
    with Builder(c):
        docker_compose_run(c, 'php bin/console make:migration', no_deps=True)
